...
class TestOpenSqlLabWithContext:
    ...
    def test_denies_inaccessible_database(self) -> None:
        mod, saved_modules = _get_tool_module()
        try:
            with (
                patch(
                    "superset.daos.database.DatabaseDAO.find_by_id",
                    return_value=Mock(database_name="warehouse"),
                ),
                patch(
                ...
                response = mod.open_sql_lab_with_context(
                    OpenSqlLabRequest(database_id=7),
                    _make_mock_ctx(),
                )
            ...
            assert response.database_id == 7
            assert response.error == sanitize_for_llm_context(
                "Access denied to database warehouse",
                field_path=("error",),
            )
        finally:
            _restore_modules(saved_modules)
