...
from superset.mcp_service.sql_lab.schemas import (
    ...
    GenerateExploreLinkFromSavedQueryResponse,
)
...
@tool(
    ...
    annotations=ToolAnnotations(
        title="Generate explore link from saved query",
        readOnlyHint=False,
        destructiveHint=False,
    ),
)
async def generate_explore_link_from_saved_query(
    request: GenerateExploreLinkFromSavedQueryRequest,
    ctx: Context,
) -> GenerateExploreLinkFromSavedQueryResponse:
    """Create a virtual dataset from a saved query and build an explore URL."""
    await ctx.info(
        "Generating explore link from saved query: saved_query_id=%s, chart_type=%s"
        % (request.saved_query_id, request.config.chart_type)
    )

    with event_logger.log_context(
        action="mcp.generate_explore_link_from_saved_query.lookup"
    ):
    ...
    with event_logger.log_context(
        action="mcp.generate_explore_link_from_saved_query.create"
    ):
        dataset = run_create_dataset_command(
            payload,
            command_factory=CreateDatasetCommand,
            action_label="Generate explore link from saved query",
        )

    if isinstance(dataset, DatasetError):
        await ctx.warning(
            "Saved-query explore-link promotion failed at dataset creation: "
            "error_type=%s, error=%s" % (dataset.error_type, dataset.error)
        )
        return GenerateExploreLinkFromSavedQueryResponse(
            dataset=None,
    ...
    if isinstance(dataset_info, DatasetError):
        await ctx.warning(
            "Saved query explore-link promotion response serialization failed: "
    ...
    await ctx.info(
        "Explore link generated from saved query: dataset_id=%s, has_error=%s"
